...
# Header(s) to mimic a real browser and bypass some basic bot protections wich would be usefull alot of times tbh
HEADERS = {
...
# this thingy is to get all links from the homepage that you can access (you could edit this to check the script and check PATH:{path thing} but i dont know how to do it and most of them will just be in href)
...
def scan_url(url, is_main_page=False):
    ...
    if xss_found and sql_found:
        print(f"Both XSS and SQL Injection vulnerabilities found on: {url}\n")
    elif xss_found:
...
def scan_website(domain):
    ...
    if not links:
        print("No additional links found, only scanned the main page.")
        return
